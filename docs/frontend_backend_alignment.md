...
- 已接入项目素材包读取、保存、确认与补充材料上传
- 已接入在线 Markdown 保存与选区 AI 改写
- 已接入章节完成、待复核、未生成统计
- 已接入当前章节证据链和审稿反馈展示
...
当前前端依赖：
- `biddingService.getReview()`
- `biddingService.getExportReadiness()`
...
- 已可真实导出
- 导出已增加“未完成章节禁止导出”的约束
- 前端已接入真实 readiness 检查，不再是静态封标清单

建议：
- 继续细化 readiness 文案和检查项
- 后面可考虑把必交材料检查做成真实配置
