...
async def wait_for_rfp_task(task_id: str, db, timeout_seconds: int = 180) -> dict:
    for _ in range(timeout_seconds):
        payload = await rfp_v2.get_task_status(task_id, db)
        if payload["status"] in {"completed", "failed"}:
            return payload
        fallback_payload = build_db_fallback_result(db)
...
def build_db_fallback_result(db) -> dict | None:
    source_doc = (
...
async def main() -> None:
    ...
    try:
        ...
        final_status = await wait_for_rfp_task(analyze_result["task_id"], db)
        print(
            {
