...
class AutoEvergreenExtractor:
    ...
    @staticmethod
    def _allows_absorb(frontmatter: dict[str, Any]) -> bool:
        value = frontmatter.get("allow_absorb", True)
    ...
    def process_file(
        ...
        try:
            ...
            if not self._allows_absorb(frontmatter):
                result["status"] = "skipped"
                result["skip_reason"] = "allow_absorb_false"
                self.logger.log("evergreen_skipped", {
                    "file": str(file_path),
                    "reason": "allow_absorb_false",
                    "bid_doc_class": frontmatter.get("bid_doc_class", ""),
                })
                return result
    ...
    if directory:
        ...
        for area in areas:
            for days_ago in range(recent):
                ...
                resolved_dir = str(date_dir.resolve())
                if date_dir.exists() and resolved_dir not in seen_dirs:
