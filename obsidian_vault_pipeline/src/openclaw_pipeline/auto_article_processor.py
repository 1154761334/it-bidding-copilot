...
try:
    from .runtime import VaultLayout, resolve_vault_dir, iter_markdown_files
...
"""
    ...
    def classify_article(self, title: str, content: str, classification_hint: str | None = None) -> str:
        """自动分类文章领域"""
        if classification_hint in {"current_tender", "historical_bid"}:
            return "programming"
        ...
        programming_keywords = ['code', 'programming', 'rust', 'python', 'javascript', 'api',
                                '编程', '代码', '开发', '架构']
        bidding_keywords = [
            '招标', '投标', '评标', '采购需求', '资格要求', '商务技术', '投标文件', '招标文件',
            '私有云建设', '评分因素', '评审因素', '资信情况'
        ]

        bidding_score = sum(1 for k in bidding_keywords if k in title_lower or k in content_lower)
        if bidding_score:
            return "programming"
    ...
    def generate_interpretation(
        ...
        """生成6维度深度解读"""
        classification = self.classify_article(title, content, classification_hint=classification_hint)
...
class AutoArticleProcessor:
    ...
    def _relative_to_raw(self, file_path: Path) -> Path:
        return file_path.resolve().relative_to(self.raw_dir.resolve())

    def _relative_to_processing(self, file_path: Path) -> Path:
        return file_path.resolve().relative_to(self.processing_dir.resolve())

    def _stage_source_for_processing(self, file_path: Path) -> Path:
        self.processing_dir.mkdir(parents=True, exist_ok=True)
        resolved = file_path.resolve()
        if resolved == self.processing_dir.resolve() or self.processing_dir.resolve() in resolved.parents:
            return file_path
        staged_path = self.processing_dir / self._relative_to_raw(file_path)
    ...
    def _restore_source_to_raw(self, file_path: Path) -> Path:
        restored = self._move_source_file(file_path, self.raw_dir / self._relative_to_processing(file_path))
    ...
    def _archive_source_to_processed(self, file_path: Path) -> Path:
        destination = self.layout.processed_month_dir(self._extract_source_date(file_path)) / self._relative_to_processing(file_path)
    ...
    def init_llm(self, api_key: str | None = None, api_base: str | None = None):
        ...
        model_name = os.environ.get("AUTO_VAULT_MODEL", DEFAULT_MINIMAX_MODEL)
    ...
    def _augment_frontmatter(
        ...
        area: str,
        txn_id: str,
        source_frontmatter: dict[str, Any] | None = None,
    ) -> str:
        """
        ...
        try:
            ...
            fm_dict: dict[str, Any] = parsed if isinstance(parsed, dict) else {}
        ...
        if source_frontmatter:
            for key in (
                ...
                if key in source_frontmatter:
                    fm_dict[key] = source_frontmatter[key]

        # Update frontmatter
        fm_dict["area"] = area
        fm_dict["canonical_concepts"] = canonical
        fm_dict["concept_candidates"] = candidates
        fm_dict["link_resolution_status"] = "resolved"
        fm_dict["link_resolution_version"] = RESOLVER_VERSION
        fm_dict["pipeline_run_id"] = txn_id

        new_fm_text = yaml.safe_dump(fm_dict, sort_keys=False, allow_unicode=True).strip()
        return f"---\n{new_fm_text}\n---\n{body}"
    ...
    def process_single_file(self, file_path: Path, dry_run: bool = False) -> dict:
        ...
        try:
            ...
            interpretation, metadata, classification = self.article_processor.generate_interpretation(
                ...
                date=file_data["date"],
                classification_hint=str(file_data.get("frontmatter", {}).get("bid_doc_class", "")).strip() or None,
            )
            ...
            interpretation = self._augment_frontmatter(
                ...
                classification,
                txn_id,
                source_frontmatter=file_data.get("frontmatter"),
            )
    ...
    def process_inbox(self, dry_run: bool = False, batch_size: int | None = None) -> dict:
        ...
        preprocessor = DocumentPreprocessor(self.vault_dir, logger=self.logger)
        preprocess_results = preprocessor.preprocess_inbox(dry_run=dry_run)
        self.logger.log("document_preprocess_batch", {
            "results": preprocess_results,
            "dry_run": dry_run,
        })
        results["preprocessed"] = preprocess_results

        processing_files = sorted(iter_markdown_files(self.processing_dir))
        raw_files = sorted(iter_markdown_files(self.raw_dir))
...
def main():
    ...
    _load_env_files(layout.vault_dir)
