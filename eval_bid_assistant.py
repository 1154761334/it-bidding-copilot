...
def main() -> int:
        ...
        check(
            ...
            all(
                ...
                for token in [
                    ...
                    "testBidSmokeAcceptanceManifestDrift.mts",
        ...
        check(
            ...
            all(
                ...
                for token in [
                    ...
                    "pnpm run check:bid-smoke-secrets && pnpm run test:bid-smoke-secrets && pnpm run test:bid-smoke-acceptance-runner && pnpm run test:bid-smoke-command-matrix && pnpm run test:bid-smoke-acceptance-manifest && pnpm run test:bid-smoke-acceptance-manifest-drift && pnpm run test:bid-smoke-preflight-summary && pnpm run test:bid-route-production-docs && pnpm run test:bid-route-production-docs-failure && pnpm run test:bid-route-production-docs-drift && pnpm run smoke:bid-route",
                ]
            )
            and all(
                token in frontend_bid_route_runbook
        ...
        check(
            ...
            and all(
                ...
                for token in [
                    ...
                    "BID_SMOKE_ACCEPTANCE_MANIFEST_PATH",
                    "MANIFEST_PATH",
                    "sub_artifacts",
                    "packageScriptName",
                    "BID_SMOKE_ACCEPTANCE_MANIFEST_TEST_PASS",
    ...
    checks.append(
        check(
            "frontend bid smoke acceptance manifest has drift fixture",
            all(
                token in frontend_bid_route_acceptance_manifest_drift_test
                for token in [
                    "BID_SMOKE_ACCEPTANCE_MANIFEST_PATH",
                    "PREFLIGHT_SCRIPT_NAME",
                    "MANIFEST_SELF_TEST_COMMAND",
                    "PREFLIGHT_PORT_GUARD_MARKER",
                    "manifestSelfTestIndex > preflightPortGuardIndex",
                    "runtime-acceptance-manifest-drift",
                    "BID_SMOKE_ACCEPTANCE_MANIFEST_DRIFT_TEST_PASS",
                    "BID_SMOKE_COMMAND_MATRIX_DRIFT_TEST_PASS",
                    "test:bid-smoke-command-matrix-drift-missing",
            ...
            and "test:bid-smoke-acceptance-manifest-drift" in frontend_package_json
            and "pnpm run test:bid-smoke-acceptance-manifest-drift" in frontend_bid_route_runbook
            and "BID_SMOKE_ACCEPTANCE_MANIFEST_DRIFT_TEST_PASS" in frontend_bid_route_runbook
            and "BID_SMOKE_ACCEPTANCE_MANIFEST_DRIFT_TEST_PASS" in frontend_bid_route_acceptance_manifest
            and "testBidSmokeAcceptanceManifestDrift.mts" in frontend_bid_route_secret_check,
        )
    )
    checks.append(
        check(
            "frontend bid smoke preflight summary is executable",
            all(
                ...
                for token in [
                    ...
                    "sub_artifact_count",
                    "terminal_status",
                    "manifest.expected_terminal_status",
                    "missingStatuses",
                    "Runbook service-free artifact row is missing statuses",
                    "preflightScript.includes(`pnpm run ${SUMMARY_SCRIPT_NAME}`)",
                    "BID_ACCEPTANCE_PREFLIGHT_ONLY=1",
                    "artifacts",
                ]
            )
            and "test:bid-smoke-preflight-summary" in frontend_package_json
            and "pnpm run test:bid-smoke-preflight-summary" in frontend_bid_route_runbook
            and "BID_SMOKE_PREFLIGHT_SUMMARY_TEST_PASS" in frontend_bid_route_runbook
            and "BID_SMOKE_PREFLIGHT_SUMMARY_TEST_PASS" in frontend_bid_route_acceptance_manifest
            and "testBidSmokePreflightSummary.mts" in frontend_bid_route_secret_check,
        )
    )
    checks.append(
        check(
